...
class AuctionActor(dataBundle:AuctionStartupData) extends Actor with FSM[AuctionState,AuctionData] with ActorLogging {
  ...
  when(Undefined) {
    case Event(startAuction(),Uninitialized) => {
      log.info(AUCTION_START)
      goto(Created) using AuctionDataBundle(dataBundle.value,dataBundle.bidTime,dataBundle.deleteTime)
    }
  }
  ...
  whenUnhandled {
    case Event(startAuction(),_) => {
      log.info("Trying to start already started auction");
      stay 
    }
    case Event(bid(newBid,newBuyer),_) => {
      sender ! stopBidding(self)
      stay
    }
    case Event(e,s) => {
